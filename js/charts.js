...
export class Charts {
    ...
    updateProgressChart(trainingHistory, customExercises) {
        ...
        if (filtered.length === 0) {
            ...
            ctx.font = '14px system-ui';
            ctx.textAlign = 'center';
            ctx.fillText('Нет данных по этому упражнению', ctx.canvas.width / 2, ctx.canvas.height / 2);
            return;
        }
        ...
        this.chart1 = new Chart(ctx, {
            ...
            options: {
                ...
                scales: {
                    y: { 
                        title: { display: true, text: '1ПМ (кг)', color: '#9ba1bc' }, 
                        grid: { color: '#2a2f40' }, 
                        ticks: { color: '#eef2ff' } 
                    },
                    x: { 
    ...
    updateWeightChart(bodyWeightHistory) {
        ...
        if (bodyWeightHistory.length === 0) {
            ...
            ctx.font = '14px system-ui';
            ctx.textAlign = 'center';
            ctx.fillText('Нет данных о весе', ctx.canvas.width / 2, ctx.canvas.height / 2);
            return;
        }
