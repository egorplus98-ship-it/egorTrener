...
export function renderExercises() {
    ...
    // Обработчики для изменения порядка (клик на номер)
    ...
    // Обработчики добавления подходов
    ...
    document.querySelectorAll('.set-input, .weight-type-select').forEach(inp => {
    ...
    for (let date in grouped) {
        ...
        grouped[date].forEach(entry => {
            ...
            entry.sets.forEach(set => {
                ...
                setsHtml += `<div class="history-set-item">▪️ ${weightDisplay} × ${set.reps} (💪${set.effort}/10)</div>`;
            });
            setsHtml += '</div>';
            exDiv.innerHTML = `
